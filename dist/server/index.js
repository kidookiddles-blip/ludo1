...
if (!jwtSecret) {
    throw new Error("JWT_SECRET environment variable is missing. Please configure it before starting the server.");
}
...
function safe(socket, action, ack) {
    try {
        action();
    }
    catch (error) {
...
io.on("connection", (socket) => {
    ...
    socket.on("moveToken", (payload) => safe(socket, () => {
        ...
        if (capture)
            io.to(state.code).emit("capture", capture);
        emitRoom(state.code);
    }));
...
setInterval(() => {
    ...
    for (const state of game.activeRooms()) {
        ...
        if (aiAction.kind === "roll") {
            ...
            if (result.capture)
                io.to(state.code).emit("capture", result.capture);
        }
        emitRoom(state.code);
    }
